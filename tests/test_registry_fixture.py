...
RADICLE_SECOND_PUBLIC_SEED_HEALTH_PATH = ROOT / "evidence" / "radicle-second-public-seed-health-2026-06-29.json"
RADICLE_PUBLIC_SEED_UPDATE_D596024_PATH = ROOT / "evidence" / "radicle-public-seed-update-d596024-2026-06-30.json"
RADICLE_PUBLIC_SEED_PRIMARY_HEALTH_D596024_PATH = ROOT / "evidence" / "radicle-public-seed-primary-health-d596024-2026-06-30.json"
RADICLE_PUBLIC_SEED_SECOND_HEALTH_D596024_PATH = ROOT / "evidence" / "radicle-public-seed-second-health-d596024-2026-06-30.json"
RADICLE_FIRST_PUBLIC_CLONE_PRIMARY_D596024_PATH = ROOT / "evidence" / "radicle-first-public-clone-primary-d596024-2026-06-30.json"
...
class RegistryFixtureTests(unittest.TestCase):
    ...
    def test_static_forge_app_is_current_and_non_publishing(self):
        ...
        self.assertEqual(app_data["projects"][0]["readiness"]["status"], "local-product-ready")
        self.assertEqual(app_data["projects"][0]["readiness"]["next_action"], "Run Nostr replay plan")
        self.assertIn("start-project ../my-project", app_data["projects"][0]["readiness"]["start_project_command"])
        self.assertIn("verify-bundle", app_data["projects"][0]["readiness"]["verify_bundle_command"])
        readiness_steps = {item["id"]: item for item in app_data["projects"][0]["readiness"]["steps"]}
        self.assertEqual(readiness_steps["local_registry"]["state"], "complete")
    ...
    def test_verification_bundle_is_current_and_self_verifying(self):
        ...
        self.assertEqual(report["first_public_clone"]["entry_count"], 4)
        ...
        self.assertIn("evidence/radicle-first-public-clone-outside-reader-ubuntu-work-primary-e0f7144-2026-06-30.json", text_report)
        self.assertIn("evidence/radicle-first-public-clone-outside-reader-ubuntu-work-second-e0f7144-2026-06-30.json", text_report)
        with tempfile.TemporaryDirectory() as tmpdir:
    ...
    def test_next_loop_controller_is_approval_bounded(self):
        ...
        self.assertEqual(controller["active_goal"]["id"], "post-alpha-public-ready-hardening")
        self.assertEqual(controller["active_goal"]["plan"], "docs/post-alpha-hardening-plan.md")
        self.assertIn("outside-reader public clone evidence", controller["active_goal"]["target_claim"])

        loop_ids = [loop["id"] for loop in controller["autonomous_loop_sequence"]]
        self.assertEqual(
            loop_ids,
            [
                "outside-reader-evidence-import",
                "latest-seed-status-surface",
                "release-decision-package",
                "collaboration-live-refresh-gate",
                "signing-provenance-hardening-gate",
                "durable-storage-gate-plan",
            ],
        )
    ...
    def test_next_loop_controller_plan_is_secret_free_and_non_live(self):
        ...
        self.assertIn("post-alpha-public-ready-hardening", result.stdout)
        self.assertIn("outside-reader-evidence-import", result.stdout)
        self.assertIn("collaboration-live-refresh-gate", result.stdout)
    ...
    def test_post_alpha_hardening_plan_defines_public_ready_gates(self):
        plan = POST_ALPHA_HARDENING_PLAN.read_text(encoding="utf-8")
        for required in [
            ...
            "e0f71448005b5ba8ed4a4e7ee55fa05b721ae23f",
            "d596024dac0d90605d4f103d567e5851771be5a8",
    ...
    def test_loop74_second_public_seed_readback_is_bounded(self):
        health = json.loads(RADICLE_SECOND_PUBLIC_SEED_HEALTH_PATH.read_text(encoding="utf-8"))

        self.assertEqual(health["schema_version"], "decentralized-forge.public-radicle-seed-health-check.v1")
        self.assertEqual(health["seed"], "z6MksRdjzuN2VYV4HTXdVSchitJ8Bq1zbx8WhBb3KhyfSm6A@187.77.19.162:8877")
        self.assertEqual(health["seed_node_id"], "z6MksRdjzuN2VYV4HTXdVSchitJ8Bq1zbx8WhBb3KhyfSm6A")
        self.assertEqual(health["rid"], "rad:z3Q8ePG6Qs4PQi1SWf9BEzDayENcy")
        self.assertEqual(health["expected_commit"], "ef16e2ad39d3e13bdcc9d454443c5bbb17733c68")
        self.assertTrue(health["auth_succeeded"])
        self.assertTrue(health["node_started"])
        self.assertTrue(health["connected_to_seed"])
        self.assertTrue(health["clone_succeeded"])
    ...
    def test_loop75_public_seed_update_to_current_head_is_bounded(self):
        evidence = json.loads(RADICLE_PUBLIC_SEED_UPDATE_D596024_PATH.read_text(encoding="utf-8"))
        primary = json.loads(RADICLE_PUBLIC_SEED_PRIMARY_HEALTH_D596024_PATH.read_text(encoding="utf-8"))
        second = json.loads(RADICLE_PUBLIC_SEED_SECOND_HEALTH_D596024_PATH.read_text(encoding="utf-8"))
        openclaw = json.loads(RADICLE_LOOP75_OPENCLAW_REFRESH_PATH.read_text(encoding="utf-8"))
        ubuntu_work = json.loads(RADICLE_LOOP75_UBUNTU_WORK_REFRESH_PATH.read_text(encoding="utf-8"))

        self.assertEqual(evidence["schema_version"], "decentralized-forge.radicle-public-seed-update-propagation.v2")
        self.assertEqual(evidence["loop"], 75)
        self.assertEqual(evidence["rid"], "rad:z3Q8ePG6Qs4PQi1SWf9BEzDayENcy")
        self.assertEqual(evidence["prior_public_seed_commit"], "ef16e2ad39d3e13bdcc9d454443c5bbb17733c68")
        self.assertEqual(evidence["updated_commit"], "d596024dac0d90605d4f103d567e5851771be5a8")
        self.assertTrue(evidence["retained_update"]["seed_restart_script_passed"])
        self.assertTrue(evidence["retained_update"]["retained_rid_advanced_to_updated_commit"])
        self.assertTrue(evidence["retained_update"]["temporary_maintainer_seed_stopped_after_sync"])
        self.assertTrue(evidence["follower_refresh"]["openclaw_storage_refresh_succeeded"])
        self.assertTrue(evidence["follower_refresh"]["ubuntu_work_rad_home_refresh_succeeded"])
        self.assertTrue(evidence["follower_refresh"]["ubuntu_work_second_seed_node_id_preserved"])
        self.assertEqual(evidence["follower_refresh"]["openclaw_readback_commit_after_refresh"], evidence["updated_commit"])
        self.assertEqual(evidence["follower_refresh"]["ubuntu_work_readback_commit_after_refresh"], evidence["updated_commit"])
        self.assertTrue(evidence["public_readbacks"]["primary_connected_to_public_seed"])
        self.assertTrue(evidence["public_readbacks"]["primary_clone_succeeded"])
        self.assertEqual(evidence["public_readbacks"]["primary_readback_commit"], evidence["updated_commit"])
        self.assertTrue(evidence["public_readbacks"]["primary_readback_matches_updated_commit"])
        self.assertTrue(evidence["public_readbacks"]["second_connected_to_public_seed"])
        self.assertTrue(evidence["public_readbacks"]["second_clone_succeeded"])
        self.assertEqual(evidence["public_readbacks"]["second_readback_commit"], evidence["updated_commit"])
        self.assertTrue(evidence["public_readbacks"]["second_readback_matches_updated_commit"])
        self.assertTrue(evidence["verification_passed"])
        self.assertFalse(evidence["secret_values_recorded"])
        self.assertFalse(evidence["retained_state_committed"])
        self.assertFalse(evidence["retained_maintainer_key_material_copied_to_public_followers"])

        self.assertTrue(primary["verification_passed"])
        self.assertEqual(primary["readback_commit"], evidence["updated_commit"])
        self.assertTrue(second["verification_passed"])
        self.assertEqual(second["readback_commit"], evidence["updated_commit"])
        self.assertTrue(openclaw["readback_matches_expected"])
        self.assertTrue(openclaw["service_active_after_refresh"])
        self.assertTrue(ubuntu_work["readback_matches_expected"])
        self.assertTrue(ubuntu_work["service_active_after_refresh"])
        self.assertTrue(ubuntu_work["node_id_preserved"])

        blob = json.dumps(evidence).lower()
        for required in [
            "stale follower cache",
            "host-local backups",
            "temporary retained maintainer seed was stopped",
            "automatic future propagation",
            "retained maintainer key material was not copied",
        ]:
            self.assertIn(required, blob)
    ...
    def test_radicle_retained_quickstart_is_evidence_bounded(self):
        ...
        self.assertEqual(model["source_evidence_id"], "loop75-radicle-public-seed-update-d596024")
        self.assertEqual(model["source_evidence_file"], "evidence/radicle-public-seed-update-d596024-2026-06-30.json")
        self.assertEqual(model["rid"], evidence["rid"])
        self.assertEqual(model["expected_commit"], evidence["updated_commit"])
        self.assertEqual(model["readback_commit"], evidence["public_readbacks"]["primary_readback_commit"])
        self.assertEqual(model["availability_mode"], "two public follower-seed update readback")
        ...
        self.assertEqual(
            {seed["id"]: seed["address"] for seed in model["public_seeds"]},
            {
                "primary": evidence["public_readbacks"]["primary_public_seed_address"],
                "second": evidence["public_readbacks"]["second_public_seed_address"],
            },
        )
        self.assertIn(f"rad node connect {evidence['public_readbacks']['primary_public_seed_address']} --timeout 30s", model["commands"])
        self.assertIn(
            f"rad clone --timeout 120s --seed {evidence['public_readbacks']['primary_public_seed_node_id']} {evidence['rid']} decentralized-forge",
            model["commands"],
        )
        ...
        self.assertIn(evidence["public_readbacks"]["primary_public_seed_address"], rendered)
        self.assertIn("availability mode: `two public follower-seed update readback`", rendered)
    ...
    def test_first_public_clone_verifier_plan_uses_latest_retained_seed_evidence(self):
        evidence = json.loads(RADICLE_PUBLIC_SEED_UPDATE_D596024_PATH.read_text(encoding="utf-8"))

        result = subprocess.run(
            [
                sys.executable,
                str(ROOT / "scripts" / "forge_registry.py"),
        ...
        self.assertEqual(plan["schema_version"], "decentralized-forge.first-public-clone-verifier.v1")
        self.assertEqual(plan["mode"], "plan")
        self.assertFalse(plan["live_actions_executed"])
        self.assertFalse(plan["secret_values_recorded"])
        self.assertEqual(plan["source_evidence_id"], "loop75-radicle-public-seed-update-d596024")
        self.assertEqual(plan["rid"], evidence["rid"])
        self.assertEqual(plan["expected_commit"], evidence["updated_commit"])
        self.assertEqual(plan["seed"]["id"], "primary")
        self.assertEqual(plan["seed"]["address"], evidence["public_readbacks"]["primary_public_seed_address"])
        self.assertIn("scripts/check_public_radicle_seed.py", plan["command"])
        self.assertIn(evidence["public_readbacks"]["primary_public_seed_address"], plan["command"])
        self.assertIn(evidence["rid"], plan["command"])
        self.assertIn(evidence["updated_commit"], plan["command"])
        ...
        self.assertEqual(second_plan["seed"]["id"], "second")
        self.assertEqual(second_plan["seed"]["address"], evidence["public_readbacks"]["second_public_seed_address"])

        combined = result.stdout.lower() + second.stdout.lower()
        for accidental_secret_marker in ["nsec1", "-----begin", "private key:", "seed phrase:", "api_token"]:
            self.assertNotIn(accidental_secret_marker, combined)
    ...
    def test_public_seed_status_artifact_is_evidence_bounded(self):
        ...
        self.assertEqual(
            {seed["id"]: seed["latest_evidence_id"] for seed in model["seeds"]},
            {
                "primary": "loop89-radicle-outside-reader-ubuntu-work-primary-e0f7144",
                "second": "loop89-radicle-outside-reader-ubuntu-work-second-e0f7144",
            },
        )
        for seed in model["seeds"]:
            ...
            self.assertIn("evidence/radicle-first-public-clone-outside-reader-ubuntu-work-", seed["latest_evidence_file"])
            self.assertIn("ubuntu-work Linux host", seed["fresh_reader_environment"])
        for required in [
    ...
    def test_loop79_first_public_clone_live_evidence_is_bounded(self):
        ...
        cases = [
            (
                "primary",
                RADICLE_FIRST_PUBLIC_CLONE_PRIMARY_D596024_PATH,
                source["public_readbacks"]["primary_public_seed_address"],
            ),
            (
                "second",
                RADICLE_FIRST_PUBLIC_CLONE_SECOND_D596024_PATH,
                source["public_readbacks"]["second_public_seed_address"],
            ),
        ]

        for seed_id, path, seed_address in cases:
            with self.subTest(seed=seed_id):
                evidence = json.loads(path.read_text(encoding="utf-8"))
                checker = evidence["checker"]

                self.assertEqual(evidence["schema_version"], "decentralized-forge.first-public-clone-verifier.v1")
                self.assertEqual(evidence["mode"], "live")
                self.assertTrue(evidence["live_actions_executed"])
                self.assertTrue(evidence["verification_passed"])
                self.assertFalse(evidence["secret_values_recorded"])
                self.assertEqual(evidence["source_evidence_id"], "loop75-radicle-public-seed-update-d596024")
                self.assertEqual(evidence["rid"], source["rid"])
                self.assertEqual(evidence["expected_commit"], source["updated_commit"])
                self.assertEqual(evidence["seed"]["id"], seed_id)
                self.assertEqual(evidence["seed"]["address"], seed_address)
                self.assertEqual(evidence["readback_commit"], source["updated_commit"])
                self.assertEqual(checker["schema_version"], "decentralized-forge.public-radicle-seed-health-check.v1")
                self.assertTrue(checker["auth_succeeded"])
                self.assertTrue(checker["node_started"])
                self.assertTrue(checker["connected_to_seed"])
                self.assertTrue(checker["clone_succeeded"])
                self.assertEqual(checker["readback_commit"], source["updated_commit"])
                self.assertTrue(checker["readback_matches_expected"])
                self.assertTrue(checker["verification_passed"])
                self.assertEqual(checker["temp_state"], "removed")
                self.assertFalse(checker["secret_values_recorded"])
                self.assertIn("scripts/check_public_radicle_seed.py", evidence["command"])
                self.assertIn("not proof of automatic future update propagation", evidence["non_claims"])

                blob = json.dumps(evidence).lower()
                for accidental_secret_marker in ["nsec1", "-----begin", "private key:", "seed phrase:", "api_token"]:
                    self.assertNotIn(accidental_secret_marker, blob)

    def test_loop89_outside_reader_ubuntu_work_clone_evidence_is_bounded(self):
        source = json.loads(RADICLE_PUBLIC_SEED_UPDATE_D596024_PATH.read_text(encoding="utf-8"))
        index_by_id = {item["id"]: item for item in self.live_evidence_index["evidence"]}
        cases = [
            (
                "primary",
                RADICLE_OUTSIDE_READER_UBUNTU_WORK_PRIMARY_E0F7144_PATH,
                "loop89-radicle-outside-reader-ubuntu-work-primary-e0f7144",
                source["public_readbacks"]["primary_public_seed_address"],
            ),
            (
                "second",
                RADICLE_OUTSIDE_READER_UBUNTU_WORK_SECOND_E0F7144_PATH,
        ...
        for seed_id, path, evidence_id, seed_address in cases:
            with self.subTest(seed=seed_id):
                evidence = json.loads(path.read_text(encoding="utf-8"))
                checker = evidence["checker"]
                index_entry = index_by_id[evidence_id]
                public = index_entry["public_identifiers"]
                ...
                self.assertEqual(evidence["seed"]["address"], seed_address)
                self.assertEqual(evidence["readback_commit"], source["updated_commit"])
                self.assertEqual(checker["host"], "atladmin-PowerEdge-R530")
                self.assertTrue(checker["connected_to_seed"])
                self.assertTrue(checker["clone_succeeded"])
                self.assertTrue(checker["readback_matches_expected"])
                self.assertTrue(checker["verification_passed"])
                self.assertEqual(checker["temp_state"], "removed")
                self.assertEqual(index_entry["evidence_file"], forge_registry.relative(path))
                self.assertEqual(index_entry["scope"], f"radicle.first_public_clone_rc.outside_reader_ubuntu_work.{seed_id}_public_seed")
                self.assertEqual(public["outside_reader_host"], "ubuntu-work")
                self.assertEqual(public["release_candidate_commit"], "e0f71448005b5ba8ed4a4e7ee55fa05b721ae23f")
                self.assertEqual(public["readback_commit"], source["updated_commit"])
                self.assertTrue(public["readback_matches_expected"])
                self.assertFalse(public["secret_values_recorded"])
                self.assertIn("not proof of automatic future update propagation", index_entry["non_claims"])
                self.assertIn("not production readiness", index_entry["non_claims"])

                blob = json.dumps(evidence).lower() + json.dumps(index_entry).lower()
                for accidental_secret_marker in ["nsec1", "-----begin", "private key:", "seed phrase:", "api_token"]:
                    self.assertNotIn(accidental_secret_marker, blob)
    ...
    def test_radicle_retained_rid_quickstart_doc_matches_latest_availability(self):
        ...
        self.assertIn(evidence["public_readbacks"]["primary_public_seed_address"], doc)
        self.assertIn(evidence["public_readbacks"]["second_public_seed_address"], doc)
        self.assertIn("evidence/radicle-public-seed-update-d596024-2026-06-30.json", doc)
        self.assertIn(f"rad node connect {evidence['public_readbacks']['primary_public_seed_address']} --timeout 30s", doc)
        self.assertIn(
            f"rad clone --timeout 120s --seed {evidence['public_readbacks']['primary_public_seed_node_id']} {evidence['rid']} decentralized-forge",
            doc,
        )
        for required_boundary in [
            "Loop 75 proved public direct-seed update readback",
            "verify-first-public-clone",
            "evidence/radicle-first-public-clone-primary-d596024-2026-06-30.json",
            "evidence/radicle-first-public-clone-second-d596024-2026-06-30.json",
            "not a durability guarantee",
    ...
    def test_first_public_clone_surface_docs_match_loop75_evidence(self):
        evidence = json.loads(RADICLE_PUBLIC_SEED_UPDATE_D596024_PATH.read_text(encoding="utf-8"))
        readme = README_PATH.read_text(encoding="utf-8")
        milestone = FIRST_DECENTRALIZED_REPO_MILESTONE.read_text(encoding="utf-8")
        combined = readme + "\n" + milestone
        ...
        for required in [
            ...
            evidence["public_readbacks"]["second_public_seed_address"],
    ...
    def test_radicle_persistent_seed_plan_is_bounded(self):
        ...
        self.assertIn(evidence["public_readbacks"]["primary_public_seed_address"], plan)
        self.assertIn("evidence/radicle-public-seed-update-d596024-2026-06-30.json", plan)
        ...
        self.assertIn(f"rad node connect {evidence['public_readbacks']['primary_public_seed_address']} --timeout 30s", plan)
        for required_boundary in [
    ...
    def test_loop26_live_evidence_index_imports_only_bounded_evidence(self):
        ...
        self.assertEqual(
            set(by_id),
            {
                ...
                "loop74-radicle-second-public-seed-readback",
                "loop75-radicle-public-seed-update-d596024",
                "loop79-radicle-first-public-clone-primary-d596024",
        ...
        radicle_second_public = by_id["loop74-radicle-second-public-seed-readback"]
        self.assertEqual(radicle_second_public["protocol"], "radicle")
        self.assertEqual(radicle_second_public["state"], "second-public-follower-seed-readback-verified")
        self.assertTrue(radicle_second_public["live_network_action"])
        self.assertTrue(radicle_second_public["local_cli_verified"])
        self.assertFalse(radicle_second_public["synthetic"])
        self.assertEqual(radicle_second_public["evidence_file"], "evidence/radicle-second-public-seed-health-2026-06-29.json")
        self.assertEqual(radicle_second_public["public_identifiers"]["rid"], radicle_second_seed["public_identifiers"]["rid"])
        self.assertEqual(
            radicle_second_public["public_identifiers"]["current_source_commit"],
            radicle_second_seed["public_identifiers"]["current_source_commit"],
        )
        self.assertEqual(
            radicle_second_public["public_identifiers"]["second_public_seed_address"],
            "z6MksRdjzuN2VYV4HTXdVSchitJ8Bq1zbx8WhBb3KhyfSm6A@187.77.19.162:8877",
        )
        self.assertTrue(radicle_second_public["public_identifiers"]["second_seed_public_relay_enabled"])
        self.assertTrue(radicle_second_public["public_identifiers"]["second_seed_public_relay_active"])
        self.assertTrue(radicle_second_public["public_identifiers"]["second_seed_public_tcp_reachable_from_windows"])
        self.assertTrue(radicle_second_public["public_identifiers"]["second_seed_public_tcp_reachable_from_ubuntu_work"])
        self.assertTrue(radicle_second_public["public_identifiers"]["fresh_reader_connected_to_second_public_seed"])
        self.assertTrue(radicle_second_public["public_identifiers"]["fresh_reader_clone_succeeded"])
        self.assertEqual(
            radicle_second_public["public_identifiers"]["fresh_reader_readback_commit"],
            radicle_second_public["public_identifiers"]["current_source_commit"],
        )
        self.assertTrue(radicle_second_public["public_identifiers"]["fresh_reader_readback_matches_expected"])
        self.assertFalse(radicle_second_public["public_identifiers"]["retained_state_committed"])
        self.assertFalse(radicle_second_public["public_identifiers"]["secret_values_recorded"])
        self.assertFalse(radicle_second_public["public_identifiers"]["retained_maintainer_key_material_copied_to_second_seed"])
        self.assertIn("not proof of permanent durability", radicle_second_public["non_claims"])
        self.assertIn("not maintainer key material on ubuntu-work", radicle_second_public["non_claims"])

        radicle_current_update = by_id["loop75-radicle-public-seed-update-d596024"]
        self.assertEqual(radicle_current_update["protocol"], "radicle")
        self.assertEqual(radicle_current_update["state"], "two-public-follower-seed-update-readback-verified")
        self.assertTrue(radicle_current_update["live_network_action"])
        self.assertTrue(radicle_current_update["local_cli_verified"])
        self.assertFalse(radicle_current_update["synthetic"])
        self.assertEqual(radicle_current_update["evidence_file"], "evidence/radicle-public-seed-update-d596024-2026-06-30.json")
        self.assertEqual(radicle_current_update["public_identifiers"]["rid"], radicle_second_public["public_identifiers"]["rid"])
        self.assertEqual(
            radicle_current_update["public_identifiers"]["prior_verified_commit"],
            radicle_second_public["public_identifiers"]["current_source_commit"],
        )
        self.assertEqual(radicle_current_update["public_identifiers"]["current_source_commit"], "d596024dac0d90605d4f103d567e5851771be5a8")
        self.assertEqual(
            radicle_current_update["public_identifiers"]["primary_public_seed_address"],
            "z6Mkk7qWfxE18R4jt5ruXmv1a7zydT9r1sY5LXx21PWySA4f@187.77.19.162:8776",
        )
        self.assertEqual(
            radicle_current_update["public_identifiers"]["second_public_seed_address"],
            "z6MksRdjzuN2VYV4HTXdVSchitJ8Bq1zbx8WhBb3KhyfSm6A@187.77.19.162:8877",
        )
        self.assertEqual(
            radicle_current_update["public_identifiers"]["primary_public_readback_commit"],
            radicle_current_update["public_identifiers"]["current_source_commit"],
        )
        self.assertTrue(radicle_current_update["public_identifiers"]["primary_public_readback_matches_expected"])
        self.assertEqual(
            radicle_current_update["public_identifiers"]["second_public_readback_commit"],
            radicle_current_update["public_identifiers"]["current_source_commit"],
        )
        self.assertTrue(radicle_current_update["public_identifiers"]["second_public_readback_matches_expected"])
        self.assertTrue(radicle_current_update["public_identifiers"]["openclaw_storage_refresh_succeeded"])
        self.assertTrue(radicle_current_update["public_identifiers"]["ubuntu_work_rad_home_refresh_succeeded"])
        self.assertTrue(radicle_current_update["public_identifiers"]["ubuntu_work_second_seed_node_id_preserved"])
        self.assertTrue(radicle_current_update["public_identifiers"]["temporary_maintainer_seed_stopped_after_sync"])
        self.assertFalse(radicle_current_update["public_identifiers"]["retained_state_committed"])
        self.assertFalse(radicle_current_update["public_identifiers"]["secret_values_recorded"])
        self.assertFalse(radicle_current_update["public_identifiers"]["retained_maintainer_key_material_copied_to_public_followers"])
        self.assertIn("not proof of automatic future update propagation", radicle_current_update["non_claims"])
        self.assertIn("not maintainer key material on the VPS", radicle_current_update["non_claims"])
        self.assertIn("not maintainer key material on ubuntu-work", radicle_current_update["non_claims"])

        first_public_clone_primary = by_id["loop79-radicle-first-public-clone-primary-d596024"]
        first_public_clone_second = by_id["loop79-radicle-first-public-clone-second-d596024"]
        for clone_entry, seed_id, seed_address in [
            (
                first_public_clone_primary,
                "primary",
                radicle_current_update["public_identifiers"]["primary_public_seed_address"],
            ),
            (
                first_public_clone_second,
                "second",
                radicle_current_update["public_identifiers"]["second_public_seed_address"],
            ),
        ]:
            with self.subTest(seed=seed_id):
                self.assertEqual(clone_entry["protocol"], "radicle")
                self.assertTrue(clone_entry["live_network_action"])
                self.assertTrue(clone_entry["local_cli_verified"])
                self.assertFalse(clone_entry["synthetic"])
                self.assertEqual(clone_entry["public_identifiers"]["rid"], radicle_current_update["public_identifiers"]["rid"])
                self.assertEqual(
                    clone_entry["public_identifiers"]["current_source_commit"],
                    radicle_current_update["public_identifiers"]["current_source_commit"],
                )
                self.assertEqual(clone_entry["public_identifiers"]["seed_id"], seed_id)
                self.assertEqual(clone_entry["public_identifiers"]["seed_address"], seed_address)
                self.assertEqual(
                    clone_entry["public_identifiers"]["source_evidence_id"],
                    "loop75-radicle-public-seed-update-d596024",
                )
                self.assertTrue(clone_entry["public_identifiers"]["live_actions_executed"])
                self.assertTrue(clone_entry["public_identifiers"]["connected_to_seed"])
                self.assertTrue(clone_entry["public_identifiers"]["clone_succeeded"])
                self.assertEqual(
                    clone_entry["public_identifiers"]["readback_commit"],
                    radicle_current_update["public_identifiers"]["current_source_commit"],
                )
                self.assertTrue(clone_entry["public_identifiers"]["readback_matches_expected"])
                self.assertTrue(clone_entry["public_identifiers"]["temp_state_removed"])
                self.assertFalse(clone_entry["public_identifiers"]["retained_state_committed"])
                self.assertFalse(clone_entry["public_identifiers"]["secret_values_recorded"])
                self.assertFalse(clone_entry["public_identifiers"]["maintainer_seed_required"])
                self.assertIn("not proof of automatic future update propagation", clone_entry["non_claims"])
                self.assertIn("not production readiness", clone_entry["non_claims"])
        self.assertIn(
            "not proof of independent provider or network availability",
            first_public_clone_second["non_claims"],
        )
    ...
    def test_renderer_outputs_expected_html(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIn("Counts by evidence scope", html)
            ...
            self.assertIn("local fixture</span>: <strong>2</strong>", html)
            self.assertIn("source-inspected mapping</span>: <strong>1</strong>", html)
            self.assertIn("synthetic local fixture</span>: <strong>2</strong>", html)
            self.assertIn("verification-state-local-fixture", html)
            self.assertNotIn("live_verified=true</span>: <strong>1</strong>", html)
            self.assertIn("<dt>Evidence scope</dt><dd>local fixture</dd>", html)
            self.assertIn("<dt>Evidence scope</dt><dd>source-inspected mapping</dd>", html)
            self.assertIn("<dt>Evidence scope</dt><dd>synthetic local fixture</dd>", html)
    ...
    def test_renderer_can_include_local_nip34_adapter_fixture_section(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIn("live_verified=false</span>: <strong>5</strong>", html)
            self.assertIn("bounded live evidence</span>: <strong>1</strong>", html)
            self.assertIn("local fixture</span>: <strong>3</strong>", html)
            self.assertIn("source-inspected mapping</span>: <strong>1</strong>", html)
            self.assertIn("synthetic local fixture</span>: <strong>1</strong>", html)
            self.assertIn("<dt>Evidence scope</dt><dd>bounded live evidence</dd>", html)
