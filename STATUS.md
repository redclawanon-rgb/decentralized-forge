...
Current post-alpha loop: the latest workbench loop extended the Evidence view with a customer trust path summary, protocol/evidence-scope grouping, and a next-gate callout that currently points at artifact public readback as the highest-value customer trust gap. This is derived from existing evidence rows only; it does not publish, pin, sign, fetch, or upgrade durability/security/production claims. The highest-value remaining gates are release decision/tagging, independent-provider seed availability, broader decentralized collaboration beyond selected-relay Nostr proof, signed release/provenance hardening, and durable storage/pinning with separate approval where required.
...
- Loop 74: second public seed readback - complete as public TCP `187.77.19.162:8877` is reachable from Windows and `ubuntu-work`, and a fresh `ubuntu-work` Radicle reader connected to `z6MksRdjzuN2VYV4HTXdVSchitJ8Bq1zbx8WhBb3KhyfSm6A@187.77.19.162:8877`, cloned the retained RID, and verified HEAD `ef16e2ad39d3e13bdcc9d454443c5bbb17733c68`. Boundary: the second seed process/state lives on `ubuntu-work`, but public ingress still relays through `openclaw`.
