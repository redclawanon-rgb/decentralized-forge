...
As of Loop 89, the project has a first public clone release-candidate path for retained RID `rad:z3Q8ePG6Qs4PQi1SWf9BEzDayENcy`. The current verified retained-RID commit is `d596024dac0d90605d4f103d567e5851771be5a8`, served by the primary public follower seed `z6Mkk7qWfxE18R4jt5ruXmv1a7zydT9r1sY5LXx21PWySA4f@187.77.19.162:8776` and the second public follower seed `z6MksRdjzuN2VYV4HTXdVSchitJ8Bq1zbx8WhBb3KhyfSm6A@187.77.19.162:8877`. Loop 75 advanced the retained RID to that commit, refreshed stale follower caches with host-local backups, stopped the temporary retained maintainer seed, and verified fresh public readbacks from both public seed addresses. Loop 79 ran `python scripts/forge_registry.py verify-first-public-clone` from fresh Docker/Linux reader profiles against both public seed addresses. Loop 89 then ran the same verifier from a separate `ubuntu-work` Linux host and a fresh checkout at commit `e0f71448005b5ba8ed4a4e7ee55fa05b721ae23f`; both public seed addresses cloned the retained RID and read back the same expected commit. The exact reader path is documented in `docs/radicle-retained-rid-quickstart.md` and emitted by `python scripts/forge_registry.py radicle-retained-quickstart`.
...
| Radicle | Current public direct-seed clone path for retained RID `rad:z3Q8ePG6Qs4PQi1SWf9BEzDayENcy` at commit `d596024dac0d90605d4f103d567e5851771be5a8`, served by primary seed `z6Mkk7qWfxE18R4jt5ruXmv1a7zydT9r1sY5LXx21PWySA4f@187.77.19.162:8776` and second seed `z6MksRdjzuN2VYV4HTXdVSchitJ8Bq1zbx8WhBb3KhyfSm6A@187.77.19.162:8877`; retained maintainer lane; follower-seed service evidence; repeatable public seed health check; first public clone verifier command; fresh Docker/Linux public clone proof; separate `ubuntu-work` outside-reader public clone proof; and a read-only retained RID quickstart command | Permanent durability, automatic future update propagation, future default public-routing availability, broad Radicle network availability, independent provider availability for the second seed, identity trust, security, production readiness |
...
| Verification labels | Top-level and NIP-34 adapter `verification_states[]` records identify local fixtures, source-inspected mappings, synthetic fixtures, live-unverified scopes, and narrow live-verified evidence, with rendered state, evidence-scope, and claim-boundary summaries | Implicit trust, unsupported scope expansion, censorship-proof/durability/security guarantees |
...
- `evidence/radicle-ubuntu-work-follower-bootstrap-2026-06-29.json` and `evidence/radicle-second-seed-tailnet-health-2026-06-29.json` — Loop 73 second persistent `ubuntu-work` follower seed bootstrap plus `openclaw` fresh-reader Tailnet readback for the same retained RID and commit
- `evidence/radicle-second-public-seed-health-2026-06-29.json` — Loop 74 second public direct-seed readback through `z6MksRdjzuN2VYV4HTXdVSchitJ8Bq1zbx8WhBb3KhyfSm6A@187.77.19.162:8877`
- `evidence/radicle-public-seed-update-d596024-2026-06-30.json` — Loop 75 retained RID update to commit `d596024dac0d90605d4f103d567e5851771be5a8` with refreshed follower caches and fresh readbacks from both public seed addresses
- `evidence/radicle-public-seed-primary-health-d596024-2026-06-30.json` and `evidence/radicle-public-seed-second-health-d596024-2026-06-30.json` — Loop 75 fresh-profile public health checks for the primary and second public seed addresses
- `evidence/radicle-first-public-clone-primary-d596024-2026-06-30.json` and `evidence/radicle-first-public-clone-second-d596024-2026-06-30.json` — Loop 79 fresh Docker/Linux `verify-first-public-clone` evidence for the primary and second public seed addresses
- `evidence/radicle-first-public-clone-outside-reader-ubuntu-work-primary-e0f7144-2026-06-30.json` and `evidence/radicle-first-public-clone-outside-reader-ubuntu-work-second-e0f7144-2026-06-30.json` — Loop 89 separate `ubuntu-work` outside-reader `verify-first-public-clone` evidence from a fresh checkout at commit `e0f71448005b5ba8ed4a4e7ee55fa05b721ae23f`
- `docs/first-decentralized-repo-milestone.md` — first usable public Radicle direct-seed clone milestone and non-claims
- `docs/first-public-clone-rc-plan.md` — autonomous Loops 77-82 plan for turning the public direct-seed path into a v0.1.0-alpha release candidate
...
- `scripts/render_forge_app.py` — stdlib generator for `output/forge-app.html`, a local static workbench over registry fixtures, scoped readiness evidence, customer-facing evidence trust-path grouping, selected-relay Nostr readback, and unsigned local Nostr issue/patch drafts
...
Verify the first public Radicle clone path without copying maintainer state:
...
The renderer summarizes both registry-level and adapter-level verification rows: total row counts, live-verified false/true counts, synthetic false/true counts, state chips, derived evidence-scope counts, grouped rows by state, and claim-boundary summaries. The workbench dashboard reuses that evidence-scope vocabulary for readiness steps, so local fixture, synthetic fixture, source-inspected, planned/live-unverified, and bounded live evidence are not collapsed into one generic readiness status. The Evidence view groups committed evidence by protocol and evidence scope, then highlights artifact public readback as the current next customer-trust gate because clone evidence is stronger than public artifact availability evidence. It also displays a concise conformance summary without dumping full serialized event payloads by default. Further public gateway checks, pinning, paid storage, broader/repeated Radicle public-network testing, or stronger durability/censorship/security/production claims require separate evidence and approval.
