...
**Next:** Loop 21 should be a hard prerequisite gate or purely local docs/test cleanup while `rad` remains unavailable. If an approved Radicle CLI path appears, record only `command -v rad` and `rad --version` evidence first and update the replay plan before any local replay. Otherwise keep work local by tightening checklist/docs/tests. Do not execute relay publishing, signing, public CI/status creation, Radicle seed/network actions, paid infrastructure, production/private key use, direct outreach, or live-verification claims unless all prerequisite gates are explicitly satisfied and command/network evidence is recorded.
