...
That path now uses the public `openclaw` seed address from Loop 72 and verifies the clone by checking `git rev-parse HEAD` against the expected commit printed by the helper.
