...
Loop 11 adds a stdlib-only local parser/export seam in `scripts/nip34_adapter.py`. The adapter reads the repository announcement and collaboration fixtures and round-trips them back into registry-shaped concepts for tests and future UI integration. Loop 12 wires that adapter into the static renderer through paired optional fixture arguments. Loop 13 adds a local repository state/status fixture generated from the recorded local Git HEAD at fixture creation time; later commits may make that recorded SHA an ancestor rather than current `HEAD`. Loop 14 adds local NIP-01 event-shape/conformance metadata reports for those dry-run events, including compact serialized payloads and possible reference event IDs when the fixture shape permits them. Loop 17 aligns adapter exports with the top-level `verification_states[]` vocabulary so imported fixture evidence carries the same local/source-inspected/synthetic/live-unverified labels as registry fixtures. Loop 18 renders registry and adapter verification-state summaries with row counts, live/synthetic flag counts, state chips, grouped rows, and claim-boundary summaries. None of these paths replace fixture IDs, sign events, verify keys, connect to relays, fetch relay state, or publish anything.
...
- `verification_states[]` rows using the same registry vocabulary for adapter-local repository announcement, collaboration, conformance, repository state, and synthetic status/check evidence.
...
When the repository and collaboration arguments are provided, the renderer imports the fixture pair through `scripts/nip34_adapter.py` and adds a **NIP-34 fixture adapter** section to the generated HTML. Loop 13's optional state/status argument extends that section with the `kind: 30618` state, HEAD ref/commit, local refs, fixture-only status/check projections, and explicit state/status non-claims. Loop 15 renders the Loop 14 conformance metadata as a concise summary: total report count, known NIP-34 kind count, and per-event label, kind, local fixture validity, placeholder id/signature flags, signed/published false fields, and `possible_event_id` where present as **local reference only**. Loop 17 also renders adapter-exported verification-state rows separately from the registry's top-level verification states. Loop 18 improves both registry and adapter verification-state display with total row counts, live-verified counts (currently zero for the fixtures), live-unverified/local counts, synthetic/non-synthetic counts, state chips, grouped rows by state, and claim-boundary summaries. The renderer intentionally does not dump full `serialized_event_payload` values by default to keep the static page low-noise.

The section displays repository id/name/kind, relay hints, dry-run publish status, imported issue and patch counts/titles/statuses/summaries/source kinds, placeholder event IDs/signatures, relay-tool fallback, synthetic key policy, NIP-35 boundary fields, repository state/status dry-run fields, and the local conformance summary.
